...
export default function App() {
  const { currentUser, loading: authLoading, logout, isAdmin } = useAuth();
  ...
  // Active month tab
  const [activeTabId, setActiveTabId] = useState<string>('setembro');
  ...
  // Load from Firestore on initial login
  useEffect(() => {
    ...
    const fetchDb = async () => {
      ...
      try {
        ...
        if (isMounted && dbMonths && dbMonths.length > 0) {
          setMonths(dbMonths);
          // Set initial tab if current active is not present
          if (!dbMonths.some((m) => m.id === activeTabId)) {
            setActiveTabId(dbMonths[0].id);
          }
          setIsDbLoaded(true);
        }
      } catch (err) {
  ...
  const handleInitialStockChange = (updated: StockItem[]) => {
    if (!isAdmin) {
      alert('Apenas usuários com perfil de Administrador podem alterar/adicionar quantidades no estoque inicial.');
      return;
    }
    updateCurrentMonth(
  ...
  const handleReformedStockChange = (updated: StockReformedItem[]) => {
    if (!isAdmin) {
      alert('Apenas usuários com perfil de Administrador podem alterar/adicionar quantidades no estoque inicial de reformados.');
      return;
    }
    updateCurrentMonth(
  ...
  const handleControllersChange = (updated: ControllerItem[]) => {
    if (!isAdmin) {
      alert('Apenas usuários com perfil de Administrador podem alterar/adicionar quantidades no estoque de controladoras.');
      return;
    }
    updateCurrentMonth(
  ...
  const handleAddStockFromModal = (
    ...
    if (!isAdmin) {
      alert('Acesso negado: Somente administradores podem adicionar quantidades ao estoque.');
      return;
    }
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 items-start">
                  ...
                  <div className="w-full">
                    <TopRightStockTable
                      ...
                      onSelectCell={handleSelectCell}
                      selectedCell={selectedCellRef}
                      canEditInitialStock={isAdmin && !isCurrentMonthFinalized}
