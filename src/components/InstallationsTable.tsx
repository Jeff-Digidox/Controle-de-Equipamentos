...
export const InstallationsTable: React.FC<InstallationsTableProps> = ({
  ...
  const handleSelectStatus = (index: number, newStatus: InstallationStatus) => {
    ...
    // usuário não administrador não pode mais alterá-la.
    if (prevStatus === 'Enviado/Instalado' && newStatus !== 'Enviado/Instalado') {
      if (!isAdmin) {
        alert('Apenas usuários administradores têm permissão para alterar o status de uma instalação que já está como "Enviado/Instalado".');
        return;
      }
    }
  ...
  return (
    <div className="flex flex-col h-full bg-white border-2 border-black rounded shadow-sm overflow-hidden text-xs">
                  ...
                  <td className="p-0 border-r border-slate-300 text-center">
                    <div className="relative w-full h-full flex items-center">
                      <select
                        ...
                        disabled={!isAdmin && row.status === 'Enviado/Instalado'}
                        ...
                        className={`w-full h-full px-2 py-1 text-center font-medium border-0 outline-none appearance-none ${statusCfg.bg} ${statusCfg.text} transition-colors ${
                          !isAdmin && row.status === 'Enviado/Instalado'
                            ? 'cursor-not-allowed opacity-95 select-none'
                            : 'cursor-pointer'
                        }`}
                        title={
                          !isAdmin && row.status === 'Enviado/Instalado'
                            ? "Status 'Enviado/Instalado' bloqueado. Apenas usuários administradores podem alterar."
                      ...
                      {/* Lock icon for non-admin when status is Enviado/Instalado */}
                      {!isAdmin && row.status === 'Enviado/Instalado' && (
                        <span
      ...
      {/* Confirmation Modal for 'Enviado/Instalado' */}
      {statusConfirmModal && (
          ...
          <div className="bg-white rounded-lg shadow-2xl max-w-md w-full overflow-hidden border border-slate-300">
            ...
            <div className="p-4 space-y-3.5 text-xs text-slate-700">
              ...
              <div className="p-3 bg-red-50 border border-red-200 rounded text-red-950 space-y-2">
                ...
                <p className="text-[11px] text-red-800 leading-normal">
                  Após confirmar, este registro ficará permanentemente bloqueado para o usuário analista. Esta alteração só poderá ser revertida ou modificada pelo <b>usuário administrador</b>.
                </p>
              </div>
            ...
            <div className="bg-slate-50 px-4 py-3 border-t border-slate-200 flex items-center justify-end gap-2.5">
              ...
              <button
                ...
                className="px-4 py-1.5 text-xs font-bold text-black bg-[#92d050] hover:bg-[#82bd44] rounded shadow flex items-center gap-1.5 transition-colors cursor-pointer border border-[#71a63b]"
              >
                <Check className="w-3.5 h-3.5" /> Sim, confirmar alteração
              </button>
            </div>
          </div>
        </div>
