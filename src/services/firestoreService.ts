...
export const DEFAULT_ANALYST_PASS = 'Analista@2026';
...
/**
 * Ensures the analista@digidox.net user (Operator role - no permission to add/edit stock) is provisioned in Firestore
 */
export async function ensureDefaultAnalystUser(): Promise<AppUser> {
  ...
  try {
    ...
    if (!userSnap.exists()) {
      ...
      const data = userSnap.data();
      if (data.role !== 'operator' || !data.password) {
        await setDoc(
          userRef,
          {
            role: 'operator',
...
export async function saveMonthToDb(
  ...
  if (!isAdmin) {
    // 1. Analista não pode criar novas abas
    const existingSnap = await getDoc(monthRef);
    if (!existingSnap.exists()) {
      throw new Error('Permissão negada: Usuário analista não pode criar novas abas no banco de dados.');
    }

    // 2. Não pode alterar status nem excluir registro que já esteja com status 'Enviado/Instalado'
    ...
    for (const oldInst of oldInstallations) {
      if (oldInst.status === 'Enviado/Instalado') {
        ...
        if (found.status !== 'Enviado/Instalado') {
          throw new Error(`Permissão negada: A instalação "${oldInst.obra || oldInst.chamado}" já está com status "Enviado/Instalado" e não pode ser alterada por analistas. Apenas o administrador tem permissão.`);
        }
      }
    }
  }
